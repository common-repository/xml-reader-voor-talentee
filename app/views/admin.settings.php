...
<div id="app-container">
	<section class="panel app">
		<div id="header">
			<header class="panel-heading">
				<h2 class="panel-title pull-left">Feedreader voor Talantee Instellingen</h2>
				<div class="clearfix"></div>
				<ul class="nav nav-tabs">
					<li class="active">
						<a href="#dashboard-doc"><span class="dashicons dashicons-dashboard"></span>
							<?php echo __('Dashboard', 'talentee-feedreader'); ?>
						</a>
					</li>
					<li>
						<a href="#settings-doc"><span class="dashicons dashicons-admin-generic"></span>
							<?php echo __('Instellingen', 'talentee-feedreader'); ?>
						</a>
					</li>
					<li>
						<a href="#advanced-settings-doc"><span class="dashicons dashicons-admin-settings"></span>
							<?php echo __('Geavanceerd', 'talentee-feedreader'); ?>
						</a>
					</li>
					<li>
						<a href="#settings-log"><span class="dashicons dashicons-media-text"></span>
							Logboek
						</a>
					</li>
				</ul>
		...
		<div class="panel-body">
			<div id="dashboard-doc">
				<?php if (defined('TFR_UPDATE')): ?>
					<div class="update-panel">
						<h4><?php echo __('Instellingen opgeslagen!', 'talentee-feedreader'); ?></h4>
				...
				<h2><?php echo __( 'Basis instellingen', 'talentee-feedreader'); ?>:</h2>
				<form class="form" id="tfr-settings-form" action="<?php echo get_admin_url('', 'edit.php?post_type='.TFR_CPT.'&page=tfr_settings'); ?>" method="POST">
					<div class="row">
						<div class="pull-left col-left">
							<label for="feedUrl">
								<?php echo __('XML Feed URL', 'talentee-feedreader'); ?>
								<span class="has-tip dashicons dashicons-editor-help"></span>
								<span class="tip-data">Voer hier de URL in naar de XML feed.</span>
							</label>
						</div>
						<div>
							<input type="text" name="feedUrl" id="feedUrl" value="<?php echo tfrSettings::get('feedUrl'); ?>" />
						</div>
					</div>
					<div class="row">
						<div class="pull-left col-left">
							<label for="officeID">
								<?php echo __('Office ID (optional)', 'talentee-feedreader'); ?>
								<span class="has-tip dashicons dashicons-editor-help"></span>
								<span class="tip-data">Voer hier de verstiging ID in om verder te filteren binnen de XML Feed. Laat dit veld leeg wanneer dit niet nodig is.</span>
							</label>
						</div>
						<div>
							<input type="text" name="officeID" id="officeID" value="<?php echo tfrSettings::get('officeID'); ?>" />
						</div>
					</div>
					
					<div class="row">
						<div class="pull-left col-left">
							<label for="cptKeyword">
								<?php echo __('URL Keyword', 'talentee-feedreader'); ?>
								<span class="has-tip dashicons dashicons-editor-help"></span>
								<span class="tip-data">Voer hier het sleutelwoord voor de URL in. Zorg er voor dat deze sleutel uniek is. De url ziet er dan als volgt uit: http://website.nl/sleutelwoord/titel-van-de-vacature.</span>
							</label>
						</div>
						<div>
							<input type="text" name="cptKeyword" id="cptKeyword" value="<?php echo tfrSettings::get('cptKeyword'); ?>" />
						</div>
					</div>

					<div class="row">
						<div class="column column-50">
							<input class="btn btn-primary" id="tfr-submit" type="submit" name="tfr-submit" value="<?php echo __('Opslaan', 'talentee-feedreader'); ?>" />
						</div>
						<?php if(tfrSettings::get('feedUrl') != '' || tfrSettings::get('feedUrl') != false) :?>
							<div class="column column-50">
								<button id="manual-update" class="btn btn-secondary right"><?php echo __('Handmatige update', 'talentee-feedreader'); ?></button>
								
							</div>
						<?php endif; ?>
						<div class="column column-100" id="manual-update-holder" style="display: none;">
							<pre>Update gestart..</pre>
					...
					<script>
						jQuery(document).on("ready", function(){
							jQuery(".update-panel .close-button").on("click", function(){
								jQuery(".update-panel").slideUp();
							});

							jQuery(document).ready(function($) {
								jQuery("button#manual-update").click(function(e){
									e.preventDefault();
									jQuery(this).slideUp();
									jQuery("#manual-update-holder").slideDown();
									var data = {
										'action': 'tfrAjaxUpdate',
										'type': 'manual'
									};

									jQuery.post(ajaxurl, data, function(response) {
										response = jQuery.parseJSON(response);
										jQuery("#manual-update-holder").append('<pre>Update voltooid!</pre>');
										jQuery("#manual-update-holder").append('<pre>Totaal aantal vacatures: '+response['totalLines']+'</pre>');
										jQuery("#manual-update-holder").append('<pre>Nieuwe vacatures: '+response['new']+'</pre>');
										jQuery("#manual-update-holder").append('<pre>Geupdate vacatures: '+response['updates']+'</pre>');
										jQuery("#manual-update-holder").append('<pre>Fouten: '+response['errors']+'</pre>');
				...
				<hr>
				<h2><?php echo __('Dashboard', 'talentee-feedreader'); ?></h2>
				<p>Welkom in het dashboard van de Talentee Feedreader plugin.</p>
				...
				<p>Je gebruikt momenteel versie <?php echo TFR_VERSION; ?> van de plugin.</p>
			...
			<div id="settings-doc" style="display:none;">
			
				<h2><?php echo __( 'Settings', 'talentee-feedreader'); ?>:</h2>
				<form class="form" id="tfr-settings-form" action="<?php echo get_admin_url('', 'edit.php?post_type='.TFR_CPT.'&page=tfr_settings'); ?>" method="POST">
					<input type="hidden" name="checkboxesAreEvil" value="1">
					<div class="row">
						<div class="column column-100 callout">
							<p><strong>Let op:</strong> Wanneer een van deze onderstaande instellingen veranderd worden, is het noodzakelijk om deze plugin te deactiveren en opnieuw te activeren.</p>
							<p>Wanneer er gebruik wordt gemaakt van de server CRON mogelijkheden, stel deze dan op de volgende manier in:</p>
							<pre>01 03 * * * /usr/bin/curl http://www.websiteurl.nl/urlKeyword?updateSecret=R97ArcTgmFnBhD</pre>
							<p>Bovenstaande zorgt er voor dat er elke nacht om 3 uur de update wordt uitgevoerd. Alleen de website url moet aangepast worden en de URL keyword (zie vorige tab).</p>
						</div>
					</div>
					<div class="row">
						<div class="pull-left col-left">
							<label for="deleteOnExpiry">
								<?php echo __('Verwijder posts wanneer de einddatum is bereikt', 'talentee-feedreader'); ?>
								<span class="has-tip dashicons dashicons-editor-help"></span>
								<span class="tip-data">Wanneer deze optie aan staat worden posts automatisch verwijderd wanneer de einddatum van de vacature bereikt is. (Let op: heractivering plugin noodzakelijk)</span>
							</label>
						</div>

						<div>
							<input type="checkbox" name="deleteOnExpiry" value="1" <?php if(tfrSettings::get('deleteOnExpiry')) { echo 'checked'; } ?>/>
						</div>
					</div>

					<div class="row">
						<div class="pull-left col-left">
							<label for="useWPSchedule">
								<?php echo __('Gebruik wp_schedule_event voor automatische updates', 'talentee-feedreader'); ?>
								<span class="has-tip dashicons dashicons-editor-help"></span>
								<span class="tip-data">Wanneer een server geen Cronjobs mogelijkheden heeft, of wanneer hier geen toegang tot is, is het ook mogelijk om automatische updates uit te voeren via Wordpress. Vink deze optie aan om via Wordpress zelf automatische updates uit te voeren. Het nadeel van deze optie, is dat bezoekers van de website hier last van kunnen ondervinden als het een grote update betreft. (Let op: heractivering plugin noodzakelijk)</span>
								
							</label>
						</div>
						<div>
							<input type="checkbox" name="useWPSchedule" value="1" <?php if(tfrSettings::get('useWPSchedule')) { echo 'checked'; } ?> />
						</div>
					</div>

					<div class="row">
						<div class="pull-left col-left">
							<label for="tfrLogging"><?php echo __('Keep logs', 'talentee-feedreader'); ?></label>
						</div>
						<div>
							<input type="checkbox" name="tfrLogging" value="1" checked disabled />
						</div>
					</div>

					<div class="row">
						<div class="column-25">
							<input class="btn btn-primary" id="tfr-submit-settings" type="submit" name="tfr-submit-settings" value="<?php echo __('Opslaan', 'talentee-feedreader'); ?>"/>
						</div>
					</div>
				</form>
			...
			<div id="advanced-settings-doc" style="display: none;">
				<div class="row">
					<div class="pull-left col-left column-25">
						<p>
							<span class="has-tip dashicons dashicons-editor-help"></span> <?php echo __('PHP Version', 'talentee-feedreader'); ?>
							<span class="tip-data"><?php echo __('Tenminste PHP 5.1.0 is noodzakelijk voor deze plugin.', 'talentee-feedreader'); ?></span>
						</p>
					</div>
					<div class="column-75"><p>
						<span class="dashicons <?php echo (tfrSettings::checkPHPVersion()) ? 'dashicons-yes' : 'dashicons-warning'; ?>"></span> 
			...
			<div id="settings-log" style="display: none;">
				<div class="row">
					<?php $log = tfrLogger::logToArray(); ?>
					<?php if (count($log) > 1): ?>
						<h3>Laatste <?php echo count($log); ?> log regels</h3>
						<ul>
					...
					<?php else: ?>
						<h3>Er zijn nog geen log regels toegevoegd.</h3>
					<?php endif; ?>					
				</div>
			</div>
			...
			<div class="footer">
				...
				<div class="info">
					<div class="info-container">
						...
						<strong>w.</strong> <a href="https://tussendoor.nl/" target="_blank">http://tussendoor.nl</a>
					</div>
				</div>
			</div>
		</footer>
	...
	<!-- <section class="panel feed">
		<div id="header">
			<header class="panel-heading">
				<h2 class="panel-title"><?= __('Recent news on Tussendoor.nl', 'talentee-feedreader'); ?></h2>
			</header>
	...
	<section class="panel feed">
		<div id="header">
			<header class="panel-heading">
				<h2 class="panel-title"><?= __('Like us on Facebook', 'talentee-feedreader') ?></h2>
				<p><?= __('Like us on Facebook and stay tuned on all our updates.', 'talentee-feedreader') ?></p>
			</header>
